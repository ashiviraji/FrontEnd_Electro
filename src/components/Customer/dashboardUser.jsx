    ...
    Axios.get(
      ...
      .then((response) => {
        if (response.data.status) {
          setNormalBillPlanCount(
            parseInt(response.data.data.result1[0].normalBill_count)
          );
          setspecialBillPlanCount(
            parseInt(response.data.data.result2[0].specialBill_count)
          );
  ...
  return (
    <div className="home-user-main">
      ...
      <div className="user-cards-area">
        <Link to="/my-bill-plans">
          <Card className={classes.currentRoot} id="card1">
            <CardContent>
              <label className="card-title-name">
                Current Available Bill Plans
              </label>
            </CardContent>
            <div>
